# add a short lewd story at the end (optional)
# llm: Huihui-Qwen3.5-27B-abliterated.Q6_K.gguf off
# llm: Qwen3.5-35B-A3B-heretic.Q4_K_M.gguf thinking: off (loops sometimes)
...
class Config:
    SHORT_NAME = "itlewder"    
    EPISODES_SHORTDESC_DIR = Path("summary_en") # not used
    EPISODES_DIALOGUES_DIR = Path("itlewd_split_en")
    EPISODES_LEWD_DIR = Path("episodes_Emmanuelle_eng_by_lines")
    EPISODES_TRANSLATED_DIR = Path(SHORT_NAME+"_en")    
    EPISODES_MAP = Path("chapters_split_list_4.json")   
    ...
    API_KEY = "None"  # set something (optional)
    ...
    MAX_TOKENS = 5000  # Adjust based on model context window
    temperature = 0.6
    frequency_penalty = 1.1
    
    # LLM will pick 2 tasks from 4 random tasks
    SPECIAL_TASKS = [
        "light touching, kissing",
        "undressing",
        "being naked",
        "in panties in public, topless",        
        "peeping at someone",
        "teasing, lewd poses",
        "taking a shower/bath",
        "sleeping naked",     
        "Shower Mistake",
        "Fall & grab someone by mistake",
        "Embarrassment",
        "watching via webcam, cctv",
        "Flash to strangers",      
        "having a naughty wet dream",
        "playing fith fingers or toys",
        "watching nsfw at work",
        "reading 18+ manga",
        "sleeping together",
        "doing naughty things at work",
        "taking nude pics, sending nudes to someone",
        "watching 18+ videos",      
        "setting up a only fans page",
        "finding an only fans page of some coworker woman",
        "changing in a dressing room or a toilet",
        "telling lewd jokes",
        "grabbing somebody's bottom",
        "trying on lingerie"
    ]
    NUMBER_OF_SPECIAL_TASKS_TO_PICK = 4
...
Your task:
Take a story from: story_to_change and add 2 random lewd elements/substories from the list below right into the end of the main story. Add around 300-700 words at the end of the main story. It must fit the old story, it should feel as a solid flowing story continuing from main story to a new lewd one. This new story should have a logical ending, don't live a cliff hanger at the most interesting point (no: "and the story has just begun..." - it's bad), give it an ending.
List of elements/substories to add:
{special_tasks}

Think what elements fit best into the main story, and add them. All men are straight. The new substory must be lewd and naughty!

But don't add following sex scenes: no sex.
...
story_to_change:
{dialogues_script}
END OF story_to_change.

lewd_text (it is just for your inspiration):
{lewd_text}
END OF lewd_text.

Soft cap is around 300-700 words for the new lewd substory but it's not strict, may be less or more if needed. Return the text of the chapter in English with 2 new lewd elements added (at the end), use lewd_text for your inspiration. Don't repeat parts. Now return full text: old story_to_change + new one, without any preambles or explanations.
"""
# ALSO
...
class TranslationManager:
    ...
    def read_episode_files(self, episode_id: int) -> Optional[Tuple[str, str]]:
        ...
        # Build file paths
        short_file_path = self.config.EPISODES_SHORTDESC_DIR / f"{chapter_id}.txt"
        ...
        lewd_text = lewd_text.replace("Emmanuelle", "Rachel")
    ...
    def prepare_translation_prompt(self, short_plot_description: str, dialogues_script: str, lewd_text: str) -> str:
        ...
        # (title_lewd is not used in the provided template, but kept for consistency)
        
        # pick 4 random special tasks and format them ---
        num_tasks = self.config.NUMBER_OF_SPECIAL_TASKS_TO_PICK   # = 4
        tasks_pool = self.config.SPECIAL_TASKS                    # list of available tasks
        
        # Ensure we get exactly num_tasks items (allow repeats if pool is too small)
        if num_tasks > len(tasks_pool):
            selected_tasks = random.choices(tasks_pool, k=num_tasks)   # with replacement
        else:
            selected_tasks = random.sample(tasks_pool, num_tasks)      # without replacement
        
        # Format as bullet list
        special_tasks_formatted = "\n".join([f"- {task}" for task in selected_tasks])
        print(f"Selected special tasks:\n{special_tasks_formatted}")
        # ---------------------------------------------------------
        
        # Format the full prompt with all placeholders
        # NOTE: The template is assumed to contain {special_tasks} (as shown in the user prompt).
        # Other placeholders (titles_text, short_plot_description, etc.) must exist in the actual template.
        return self.config.USER_PROMPT_TEMPLATE.format(
            titles_text=titles_text,
            special_tasks=special_tasks_formatted,      # now a bullet list
        ...
        for attempt in range(self.config.MAX_RETRIES):
            try:
                ...
                if attempt < self.config.MAX_RETRIES - 1:
                    ...
                    self.logger.info(f"Waiting {wait_time} seconds before retry (check your vpn)...")
    ...
    def get_episodes_to_process(self) -> list:
        ...
        available_episodes = []
        for episode_id in range(1, 150):  # 105 < 110 not inclusive
