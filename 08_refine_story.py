# rewrite story_en, try to fix inconsistencies
...
# Configuration
class Config:
    SHORT_NAME = "itstoryrefined"    
    EPISODES_SHORTDESC_DIR = Path("summary_en")
    EPISODES_DIALOGUES_DIR = Path("itstory_en")
...
Your task:
1. Take a story from: dialogues_script and modify it to fit the plot described in short_plot_description. Some characters in the dialogues are messed up, you should review and change who is talking what. Main thing is the plot from short_plot_description, stick to it then adapt dialogues. Current dialogues are missing transitions from scene to scene - add transitions and scene descriptions. Current logic in dialogues is also bad: characters sometimes say a things that don't belong to the plot and are out of the context. Try to refine character actions and what they are saying, use character descriptions to define their actions. Chapter should be in English.
...
Now create up with a brand new LONG chapter of the light novel in English, a little longer than original dialogues_script. MAIN TASK: refine dialogues, actions and logics. Now return just the text, without any preambles or explanations.
"""
# ALSO
