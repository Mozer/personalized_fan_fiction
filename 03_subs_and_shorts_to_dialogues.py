# ru subs-text into dialogues with names, 100 lines per file
...
class Config:
    ...
    SYSTEM_PROMPT = """You are a professional subtitle rewriter."""

    USER_PROMPT_TEMPLATE = """
light novel called "The IT crowd". 
Your task:
Rewrite dialogues from subs_text and assign them to correct character. Currently all subtiles are in a heap without character identification. Define which character is speaking. Try to guess. use short 1-2 sentences max in each line, don't make long lines. Output format: 
Character: speech.
...
No, I'll get my cell phone first.
for me, then for Roy, and then for you.
Just like your skirt.
By the way, this is my favorite one.

Output:
Jen: I don't understand what could have gone wrong.
Roy: Okay, that's it, I need a break.
Jen: Hey! Hey, that's not fair!
Jen: I want one for myself too.
Jen: No, I'll get my cell phone first. 
Jen: for me, then for Roy, and then for you.
Moss: Just like your skirt.
Jen: By the way, this is my favorite one.
...
Now return the full text with the rewritten dialogues in the desired format. Divide the dialogues into the correct characters, try to guess which character is speaking now. Characters usually speak in short phrases, 1-2 lines per character. If there are more than 2 sentences, divide them into two lines. Don't write long lines of more than 2 sentences, break them down into several lines and characters! The text will be long, please return it in full.
"""
...
class TranslationManager:
    ...
    def read_episode_files(self, episode_id: int) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            return short_plot_description, subs_text
        except Exception as e:
    ...
    # dup?
    def delete_process_episode(self, episode_id: int) -> bool:
        """Process a single episode: read both texts, translate, save"""
        # Check if already translated
        if episode_id in self.progress_data.get('completed_episodes', []):
            self.logger.info(f"Episode {episode_id} already translated, skipping...")
    ...
    def get_episodes_to_process(self) -> list:
        ...
        available_episodes = []
        for episode_id in range(1, 200):  # 105 < 110 not inclusive
