# LLM : gemma3-27B 
# LLM : translate-gemma-27B (remove USER_PROMPT)
...
# Configuration
class Config:
    SHORT_NAME = "itlewderru"
    EPISODES_FUNNY_DIR = Path("itlewder_en")
    EPISODES_TRANSLATED_DIR = Path(SHORT_NAME+"_ru")    
    OUTPUT_FILE = Path(SHORT_NAME+"_novel_trans_ru.txt")
...
Keep the tone intact and use the following name mappings: (Maurice Moss->Мосс, Maurice->Мосс, Jen Barber->Джен Барбер, Jen->Джен).
...
class TranslationManager:
    ...
    def translate_text(self, funny_text: str, episode_id: int) -> Optional[str]:
        ...
        if total_lines <= self.config.MAX_LINES_PER_PART:
            return self._translate_part(funny_text, episode_id, 1, 1)
