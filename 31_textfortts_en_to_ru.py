...
class Config:
    ...
    EPISODES_INPUT_DIR = Path("textfortts_en")
    EPISODES_TRANSLATED_DIR = Path("textfortts_ru")    
    OUTPUT_FILE = Path(SHORT_NAME+"_textfortts_trans_ru.txt")
    PROGRESS_FILE = Path("progress_translation_"+SHORT_NAME+"_ru.json")
    ...
    MAX_TOKENS = 6000  # Adjust based on model context window
    temperature = 0.5
    frequency_penalty = 1.1

    # Splitting setting
    MAX_LINES_PER_PART = 100  # split long texts into parts of max this many lines

    USER_PROMPT_TEMPLATE = """You are a professional novel translator. Translate the following text from English into Russian. 
Keep the tone intact and use the following name mappings: (Maurice Moss->Мосс, Maurice->Мосс, Jen Barber->Джен, Jen->Джен, Rachel->Рэйчел, Denholm->Денхолм).
Return ONLY the translated Russian text without any explanations, preambles, or formatting tags.
...
class TranslationManager:
    ...
    def prepare_translation_prompt(self, english_text: str) -> str:
        """Prepare the prompt for translation"""
        return self.config.USER_PROMPT_TEMPLATE.format(
            english_text=english_text
        )
    
    def _translate_part(self, part_text: str, episode_id: int, part_index: int, total_parts: int) -> Optional[str]:
        """Translate a single part of an episode with retry logic."""
        prompt = self.prepare_translation_prompt(part_text)
        
        for attempt in range(self.config.MAX_RETRIES):
            try:
                self.logger.info(f"Translating episode {episode_id}, part {part_index}/{total_parts} (attempt {attempt + 1}/{self.config.MAX_RETRIES})...")
                ...
                if translation:
                    self.logger.info(f"Successfully translated episode {episode_id}, part {part_index}")
                    return translation
                else:
                    self.logger.warning(f"Empty translation received for episode {episode_id}, part {part_index}")
                    
            except Exception as e:
                self.logger.error(f"Translation attempt {attempt + 1} failed for part {part_index}: {e}")
                ...
                if attempt < self.config.MAX_RETRIES - 1:
                    ...
                    self.logger.error(f"All translation attempts failed for episode {episode_id}, part {part_index}")
        
        return None

    def translate_text(self, english_text: str, episode_id: int) -> Optional[str]:
        """Send text to LLM for translation with retry logic. If text has more than MAX_LINES_PER_PART lines, split it."""
        lines = english_text.splitlines()
        total_lines = len(lines)
        
        # If text is short enough, translate directly
        if total_lines <= self.config.MAX_LINES_PER_PART:
            return self._translate_part(english_text, episode_id, 1, 1)
        
        # Split into parts of up to MAX_LINES_PER_PART lines
        parts = []
        for i in range(0, total_lines, self.config.MAX_LINES_PER_PART):
            part_lines = lines[i:i + self.config.MAX_LINES_PER_PART]
            parts.append('\n'.join(part_lines))
        
        total_parts = len(parts)
        self.logger.info(f"Episode {episode_id} has {total_lines} lines, splitting into {total_parts} parts for translation.")
        
        translated_parts = []
        for idx, part_text in enumerate(parts, start=1):
            translated = self._translate_part(part_text, episode_id, idx, total_parts)
            if translated is None:
                # Part failed, whole episode fails
                self.logger.error(f"Episode {episode_id} failed at part {idx}. Aborting.")
                return None
    ...
    def process_episode(self, episode_id: int) -> bool:
        ...
        # Read text
        english_text = self.read_episode_file(episode_id)
        if not english_text:
            self.save_progress(episode_id, success=False)
            return False
        
        # Translate text
        english_translation = self.translate_text(english_text, episode_id)
